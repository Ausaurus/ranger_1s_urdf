...
world_file = "empty.sdf"  # warehouse
...
def generate_launch_description():
    ...
    xacro_file = os.path.join(this_pkg_path, "urdf", robot_model + ".urdf")  # .urdf
    ...
    bridge = Node(
        ...
        arguments=[  # ign topic -t <topic_name> --info
            ...
            "/model/"
            + robot_ns
            + "/battery/linear_battery/state@sensor_msgs/msg/BatteryState[ignition.msgs.BatteryState",
        ],
        parameters=[
        ...
        remappings=[  # ign topic -l
            ...
            (
                "/world/world_model/model/" + robot_ns + "/joint_state",
                "/" + robot_ns + "/joint_states",
            ),
            # ('/'+robot_ns+'/imu', '/'+robot_ns+'/imu'),
            ("/" + robot_ns + "/lidar", "/" + robot_ns + "/scan"),
            (
                "/model/" + robot_ns + "/battery/linear_battery/state",
                "/" + robot_ns + "/battery/state",
            ),
        ],
    )
